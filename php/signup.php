<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // if email is vaild
                ...
                if(isset($_FILES['image'])){ //if file is uploaded
                    ...
                    if(in_array($img_ext, $extensions) === true){ //if user uploaded img ext is matched with any array extensions
                        $time = time(); //this will return us current time..
                                        //we need this time because when you uploading user img to in our folder we rename user file with crrent time
                                        //so all the imag file will have a unique name
                        
                        // let's move the user uploaed img to our particular folder
                        ...
                        if(move_uploaded_file($tmp_name, "images/".$new_img_name)){ //if user upload img move to our folder successfully
                            ...
                            if($sql2){ // if these data inserted
                                ...
                                if(mysqli_num_rows($sql3)>0){
                                    ...
                                    $_SESSION['unique_id'] = $row['unique_id']; //using this session we used user unique_id inother php file
                                    echo "success";
                                }                                
                            }else{
